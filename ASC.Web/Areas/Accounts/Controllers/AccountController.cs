    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Customers(CustomerViewModel customer)
        {
            ...
            if (customer.Registration.IsEdit)
            {
                ...
                var oldClaim = claims.FirstOrDefault(p => p.Type == "IsActive");
                ...
                if (oldClaim != null)
                {
                    ...
                    {
                        removeResult.Errors.ToList().ForEach(p => ModelState.AddModelError("", p.Description));
                        return View(customer);
                    }
                }
                ...
                {
                    addResult.Errors.ToList().ForEach(p => ModelState.AddModelError("", p.Description));
                    return View(customer);
                }
            }
        ...
        public async Task<IActionResult> Profile(ProfileModel profile)
        {
            ...
            if (user == null)
            {
                ModelState.AddModelError("", "Không tìm thấy tài khoản hiện tại.");
                return View(profile);
            }
